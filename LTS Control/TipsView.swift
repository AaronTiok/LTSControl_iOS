            ...
            .safeAreaInset(edge: .bottom) {
                if purchaseMessage != nil {
                    ...
                    Button(action: {
                        ...
                        Text(priceText.isEmpty ? "支持" : "Unterstützen \(priceText)")
                            .monospacedDigit()
                            .fontWeight(.semibold)
                            .frame(minWidth: 0, maxWidth: 420)
                            .padding(.vertical, 15)
    ...
    func purchase(_ product: Product) async {
        ...
        do {
            ...
            case .success(.verified(_)):
                ...
                purchaseMessage = NSLocalizedString("感谢你的支持！", comment: "Shown after a successful tip purchase")
            case .success(.unverified(_, _)):
                purchaseWasSuccessful = false
                purchaseMessage = NSLocalizedString("无法验证支付。", comment: "Shown when the transaction could not be verified")
            case .userCancelled:
                purchaseWasSuccessful = false
                purchaseMessage = nil
            case .pending:
                purchaseWasSuccessful = false
                purchaseMessage = NSLocalizedString("正在处理支付...", comment: "Shown while the payment is pending")
            @unknown default:
                purchaseWasSuccessful = false
                purchaseMessage = NSLocalizedString("购买时发生未知错误。", comment: "Shown for an unknown purchase error")
            }
        } catch {
            purchaseWasSuccessful = false
